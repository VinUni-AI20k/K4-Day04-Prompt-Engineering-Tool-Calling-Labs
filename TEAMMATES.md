...
| 4 | Nguyễn Nguyên Phong | 2A202602691 | Heargreaves1 | **UI chat + live evidence.** Xây `starter_v0/ui.py` (Streamlit/Gradio) tái dùng `run_model_tool_loop` trong `chat.py`, hiển thị user request, final response, từng tool name + args, tool result/error, round/status, artifact version + hashes, transcript path; ghi transcript vào `transcripts/`. Rehearse 3–5 scenario (normal, missing-info, multi-turn correction, ticket confirmation) và điền `REPORT.md` A1–A4, B4. |
